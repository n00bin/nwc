...
**Data-pipeline note (2026-07-17):** the served `website/data/*.js` files are AUTO-GENERATED by `website/build-data.py` from the `../data/*.json` source of truth ("Do not edit" headers). Both fixes above were applied at the JSON source and regenerated, so a future `build-data.py` run will not revert them. Static db pages regenerated via `scripts/gen-item-pages.js`.

Open follow-ups (not blocking): Incoming Damage reads −13.75% vs game −14.6% (traces to Ritualistic Necklace "Malignant Energy" 3-pc uptime weighting, not Broken Halo); sibling enchant Celestial Rime Temper (id 40) shares Shattered Resolve's HP/IDR baseline but has a DIFFERENT proc (enemy Outgoing-Damage debuff, currently inert in-engine) — left unmodeled by design.
