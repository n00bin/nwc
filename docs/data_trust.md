...
Open follow-ups (not blocking): additive→multiplicative incoming-damage stacking (above); sibling enchant Celestial Rime Temper (id 40) shares Shattered Resolve's HP/IDR baseline but has a DIFFERENT proc (enemy Outgoing-Damage debuff, currently inert in-engine) — left unmodeled by design.
