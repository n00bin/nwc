...
| gear 237, 257 | Choker of Searing Magma + Girdle of Coagulated Magma (Magmatic Efficiency 3pc) | gear | CONFIRMED stats (IL 2,800/CR 2,520); pure stat-sticks. Its setBonus ("+2% Power/Forte/Defense") stays UNVERIFIED — the captures were compact tooltips; needs an expanded-panel re-shoot | Choker/Girdle_IL2800.png | 2026.03.17a | 2026-07-09 |

Deferred (consistency): none of these accessory set bonuses is structured for SCORING (all text-only, matching the pre-existing convention) — structuring the scorable ones (Magmatic +2%×3, Dashing Decoy −5%ID/+5%Aware) is a separate all-four-sets decision.
