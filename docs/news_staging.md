...
- Gear equip bonuses now read in full, too. The same "shorthand" problem that hit set bonuses also hit each item's own **Equip:** line — many showed only a bare "+X% Stat" instead of the real effect (conditions, stacking, procs). **306 items** across every gear tier — endgame armor, weapons, clothing, and the whole accessory range (rings, necklaces, amulets) — now carry their complete, in-game-verified Equip bonus text, feeding both the builder display and the stat engine. (Most older low-level gear simply has no Equip bonus — that's correct, not missing — so those are now catalogued and no longer flagged as gaps.)
