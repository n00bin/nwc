...
### Bug Fixes
- The Maximum Hit Points formula was rebuilt from controlled in-game measurements (unslotting items one at a time on a real endgame tank and reading the exact HP change). Confirmed: flat HP from kits/insignias/food multiplies with everything else, the Constitution bonus adds to your other HP percents (it was modeled as a separate multiplier), and a combat enchantment's HP percent (Rime Temper) multiplies separately on top. Insignias and enchantments carry their full item level into TIL. The model now tracks the game within ~0.8% on the reference tank, with one small unidentified bonus still being hunted.
