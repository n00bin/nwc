...
- **(local only) Rotation builder and simulator (Toon Forge).** The Fight → Powers has an "Add step" list: add your casts in the order you use them, as many as your rotation has, including Soul Scorch, your artifact and your mount power. The tool plays it forward over your Time in combat, skipping anything on cooldown and filling gaps with your at-will, and scores the real magnitude per second instead of a per-power average. For Hellbringers it also tracks Curse on the boss, your Soul Spark pool with a "fire Soul Scorch at N" setting, the Soul Puppet and its Investiture stacks, and the burns and fields, so Warlock's Curse, Wrathful Souls and Risky Investment count at what your rotation actually produces. Executioner's Gift now follows the enemy health slider.
- **(local only) Soul Sparks live on the Class tab (Toon Forge).** One Soul Sparks control, 0 to 30, on The Fight → Class, at 0.5% damage per spark and 1.0% with Wrathful Souls. The old 0 to 10 slider on step 2 is gone locally and only shows on the live site while the Class tab is still switched off there; old saved builds carry their value across. A Soul Spark income table now sits at the bottom of the Class tab (every power, feature and the Soul Scorch spend) with a powers-only copy on step 2.
...
- **Warlock Hellbringer, every power and feat reviewed against the game (Toon Forge).** All 19 Hellbringer powers and all 29 mechanics, class features and feats were checked one at a time against tooltip screenshots and a freshly retrained level-20 character. Corrections that change numbers today: the ten encounter base cooldowns (for example Infernal Spheres 13.3 s, not 18.6), daily and encounter magnitudes now include what actually lands on a boss (Brood of Hadar 2,000 with its imps, Flames of Phlegethos 2,100 with its burn, Hellfire Ring 450 with its field), Executioner's Gift is up to 30% not 10%, Soul Sparks are 0.5% each at base and 1.0% only with Wrathful Souls, and Soul Scorch is 25 magnitude per spark at base with Double Scorch adding 25. Curse, Soul Spark income and spend, the Soul Puppet and every feat now carry the data the rotation model will read next.
...
- **Optimizer now builds neck + belt sets (members).** The optimizer could never assemble a two-piece neck and belt set from scratch, because either piece on its own is just a lower item level accessory until its partner is on. It now tries every two-piece neck/belt set as a pair after the main search (the same way it already does for shirt/pants and weapon pairs) and keeps it when the build gets better. It also tries every piece combination inside the set instead of the first by item level, because same-item-level set pieces differ by ability score. First case: the Masterwork of Menzoberranzan set now beats Voidbound on a Warlock by about 5%, and it picks the Scintillant pieces (INT/CHA) for a Warlock rather than the Shroomwood ones (STR/DEX).
- **Buff powers show their real uptime (Toon Forge).** Detailed Stats → Damage Boosts has a new "Slotted power buffs / debuffs" row: each slotted buff or debuff power (Tyrannical Curse, Lurker's Assault, Lore...) is listed as its % times the uptime your rotation gives it, for example Tyrannical Curse +15% x 33% (20 s up every 60 s Daily) = +5%. Nothing is counted as up all fight unless you tick that yourself on the Class tab.
- **(local only) Soul Investiture slider (Toon Forge Class tab).** Hellbringer Warlocks get a 0-5 stack slider for Soul Investiture. With the Risky Investment feat slotted it adds +20% Encounter damage plus +2% per stack; without the feat the row is greyed and does nothing, matching the tooltip.
- **Daily / Artifact / Mount power cadence (Toon Forge).** Three sliders on The Fight → Powers say how often you use your Daily, your artifact power and your mount combat power (about every 60 s by default, their cooldown). Every bonus that fires on one of those (Tactician's Precision, Artificer's Enticement, Cavalry's Haste, Artifact Fanatic, Daily Burst and friends) and the rotation itself read them.
- **Insignia bonuses join the review.** Insignia bonuses now use the same machinery as gear bonuses (stamina, AP, health gates, procs, cooldown cuts). First fourteen reviewed.
- **Slot your powers first (Toon Forge).** Damage and healing numbers are damage per hit times casts per second, and casts per second comes from your powers. The Results page and the optimizer now ask you to slot at least one at-will and one encounter before they score damage or healing, instead of guessing. Tank survival does not need powers.
- **Three belt slots (Toon Forge).** The Consumables tab now has three Belt Item slots, matching the game; the same item cannot be put in two slots. The In combat switch was dropped from The Fight since Detailed Stats already has it.
- **Class tab (Toon Forge) - coming soon.** The Fight → Class will hold your class mechanics (Battlerage, Soul Sparks, Curse, Vengeance) and buff/debuff powers. It shows a Coming soon note on the live site while it is finished locally.
- **(local only) Class tab lists your buff and debuff powers (Toon Forge).** Under the class mechanics, The Fight → Class now lists the powers of your class and paragon that buff you or debuff the target (Relentless Slash, Battle Fury, Adamantine Strike, Inspiration, Lore and so on). Greyed if not slotted; slotted ones you can mark "up all fight" to count them at full uptime instead of by cooldown.
- **Toon Forge reorganised into 7 steps.** Role & Class → Character & Powers → Gear (gear, artifacts, enchantments, overloads) → Companions & Mounts (companions, stable, then bolster) → Boons → The Fight (the combat scenario: you, the fight, your party and their buffs, consumables, your power mix, class mechanics, conditions) → Results (Optimize, the hit simulator, what to upgrade next, build summary). A "Fight:" line under the step bar shows the scenario you are building for on every step; click it to change it.
- **Combat scenario, round 2 (Toon Forge).** Fight tab: enemy health, enemy types you fight (several at once, they used to hide in the zone list) and Combat Advantage uptime. You tab: how long you have stood still and an In combat switch. New Powers tab shows your at-will / encounter / daily damage mix worked out from your slotted powers over the fight length. New Class tab lists your class and paragon mechanics as switches; the ones with numbers on their tooltip (Battlerage, Dig In, Vengeance, Soul Sparks, Arcane Mastery, Ranger stances) feed your stats.
- **Combat scenario panel (Toon Forge).** A new panel at the top of the builder describes the fight you are building for: You (health, action points, stamina, moving, time in combat), Fight (zone, distance, enemies, what attacks you, shielded target), Party (dungeon / trial / solo, party healer) and Conditions. Every conditional bonus on your gear reads it, and the optimizer now scores every candidate against exactly this scenario, says so at the top of its result, and tags each gear pick with the scenario inputs it relied on (for example "Brute's Fury: within 25 ft (counted at 10 ft)"). Change the scenario, run again, get the answer for that fight.
- **Equip-bonus review finished.** Every equip bonus in the gear database (148 names, from the IL 2600 Pulsar set down to level-20 starter weapons) was walked through one at a time and either given a real uptime rule, tied to a sim control or build condition, or marked as a note. Along the way 7 bad captures and 15 duplicate entries that were double-counting were removed, and 3 items got their missing halves (Movement Speed, Jerkin damage, Skirmisher defensive stats).
...
- **Clear answer when Discord sign-in does not unlock the optimizer (Toon Forge).** If Discord signs you in but your account does not hold the Legendary N00B role, the page now shows an amber box saying exactly that and how to fix it (link YouTube under Discord Settings → Connections, then re-join the server), instead of a tiny grey line. Clicking Optimize again re-shows the reason rather than silently bouncing you through Discord a second time; "Sign in again" is now an explicit link. Browsers that block site storage (private windows, some console browsers) are called out too.
...
- **Shroomwood and Scintillant Menzoberranzan pieces now carry their ability scores.** Shroomwood Amulet +6 STR, Shroomwood Sash +3 STR +3 DEX, Scintillant Amulet +6 INT, Scintillant Sash +3 INT +3 CHA (base and +1 ranks), from in-game tooltips. That ability line is the whole difference between the two sets: Shroomwood suits physical classes, Scintillant suits magic classes, and Toon Forge now values it.
