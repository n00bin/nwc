...
- Mekaniks: renamed the **Damage** tab to **Combat Formulas** and added two more theorycraft cards:
  - **Full Expected-Value Healing Formula** (collapsed) — 6-term healing formula with variable glossary, healer crit-severity penalty (÷2), and key insights (heals aren't reduced by defensive stats, stacking Incoming Healing on tanks multiplies output, Power and OH balance equally).
  - **Full Expected-Value EHP Formula** (collapsed) — defender's mirror of the damage formula. Shows the three EHP cases (Min/Avg/Max), the canonical Average formula, variable glossary, and insights (defensive stats compound multiplicatively, Awareness beats Defense above 45%, Crit Avoidance prevents tank one-shots, stat investment beats flat HP at scale).
