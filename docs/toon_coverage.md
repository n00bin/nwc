...
| Insignia bonuses on the gear equip-bonus machinery | Partial | `toon-forge.html` `_insigniaEqQueue` (ingested after the overload loop through every gate), `activeInsigniaBonuses` (proc collectors), `apAbovePct` gate; data `mount_insignia_bonuses.json` `equipBonuses[]` | Insignia bonuses can carry the same entries gear does (gates, procs, conditions, ramps) times the per-instance multiplier. 10 structured bonuses converted (Master's Cruelty + Gladiator's Guile on the stamina slider, Berserker's Rage both halves on the AP slider, Cautious Devotion on health, Trainer's Restoration on full AP). instanceStats bonuses (Ice Cold x2, Guardian's Spirit) keep the old path. 30 text-only bonuses queued for the one-at-a-time review (scripts/_insig_review_queue.py). n00b 2026-09-09. | review the 30 text-only bonuses |
