...
| You are debuffed toggle | Implemented | `toon-forge.html` `state.debuffed`, `_attackTypeSkip`, `updateDebuffedControl`; entry field `whileDebuffed`; `SIM_CONTROL_NEEDS["sim-debuffed-field"]` | Scenario panel → You checkbox (default off) gates `whileDebuffed` entries. First user: Accursed's Resolve (insignia bonus #13, +3500 Power and Deflect while debuffed), which was pinned at 90% uptime on 2026-09-09 and was winning every four-insignia slot on paper; n00b 2026-09-10: the debuffed state is rarely up, so it is never assumed. Serialized in builds + share links, in the optimizer cache key, tagged on optimizer picks ("while you are debuffed (counted/off)"). Gate covers gear, overloads and insignia bonuses (same `_attackTypeSkip`). | n/a |
| Rotation simulator (ROT-1) | Implemented, LOCAL ONLY (2026-09-11) | `toon-forge-rotation.js` (pure, node-testable) + `toon-forge.html` `TF_ROTATION_SIM_LIVE`, `rotationSimEnabled`, `buildRotationSimInput`, `getRotationSim` (cached), `peekRotationSim`; `rotationMagPerSec` / `rotationMixOverTime` switch to it when enabled | Timeline over Time in combat in 0.1 s ticks: ordered step list (skip not-ready, loop, first at-will fills), per-power cooldowns with charges + escalation + Recharge Speed, AP bar + per-daily cooldown, cast / channel times, artifact + mount cadence triggers. Output = raw magnitude/s (optimizer scores it as before) + timeline + derived uptimes. 0.4 ms per fight. Live site keeps the rate model (flag false). | flip the flag when n00b signs off; DUAL-1 modes not read yet |
| Rotation script semantics (ROT-3) | Implemented, LOCAL ONLY (2026-09-11) | `toon-forge-rotation.js` opener/loop pointer, `waitFor`, `MAX_WAIT` 5 s, `listedIn`; `toon-forge.html` `state.rotationOpener` (+ `rotationSteps` = loop), `parseRotationText`, two lists with Add step / Paste list / Clear | Opener runs once, loop repeats. Both are scripts: every entry casts in order, at-wills included; a step still on cooldown is waited for (filler at-will woven in) unless more than 5 s away, then skipped. Listed Soul Scorch casts at its turn with whatever is held (min 6); fire-at-N only auto-fires when Scorch is not in the running list. Artifact, mount power and dailies not in the running list fire on their own when up. Empty loop = opener repeats; nothing = default order. Paste list accepts one cast per line or commas, "x3" repeats, daily/artifact/mount/scorch keywords. n00b's Hellbringer opener (28 casts) + loop (14) verified in node and on the page. | n/a |
| Rotation builder UI (ROT-2) | Implemented, LOCAL ONLY (2026-09-11) | `toon-forge.html` `renderRotationBuilder`, `rotationCastables`, `#rotation-builder-box` on The Fight -> Powers; `state.rotationSteps`, `state.scorchAtSparks` (serialized, in the optimizer key) | Add-step list (any length, duplicates ok, up/down/remove, clear), fire-Soul-Scorch-at slider (6-18, default 18), readout: magnitude/s, cycles, derived chips (Curse uptime, sparks avg/peak + Scorch casts, puppet uptime + Investiture avg, Creeping Death stacks), top sources, first-cycle timeline. No list = default order (strongest per cast first) with a notice. | n/a |
| Curse model (Hellbringer) | Implemented in the simulator, LOCAL ONLY | `toon-forge-rotation.js` Curse layer reading `tags.curse` apply/consume/synergy, All-Consuming Curse, Deadly Curse, Parting Blasphemy; engine: `curseGated` features scale by `derived.curseUptime` (Warlock's Curse) | 8 s Curse from appliers, consumed by consume casts (Hadar's Grasp +150 + puppet, Curse Bite only hits when Cursed), synergy pulses x1.5 (Blades) / cursedMagnitude (Gates), Dreadtheft refreshes; Deadly Curse 25 per apply, Parting Blasphemy 85 per removal. Class-tab Curse switch is now informational. | Vengeful Curse ignored (DPS) |
| Soul Spark economy (Hellbringer) | Implemented in the simulator, LOCAL ONLY | `toon-forge-rotation.js` spark layer (`soulSparks` blocks, `perCombo`, NPNM, Dark Prayers, Dark One's Blessing, Soul Scorch spend at `scorchAtSparks`, Double Scorch, Soul Spark Recovery); engine reads `derived.sparksAvg` for the Soul Spark mechanic stacks (slider shown read-only) | Pool 0-30 from per-hit income; Scorch auto-fires at the policy unless placed in the list; hit + burn scheduled; Recovery cuts encounter cooldowns by spent/6 s. | n/a |
| Soul Puppet + Investiture (pet layer, Hellbringer) | Implemented in the simulator, LOCAL ONLY | `toon-forge-rotation.js` puppet layer (`summon` block: 60 mag @ 1/s, 20 s; Grasp consume, Soul Siphon, Power of the Nine Hells summon; re-summon = Investiture stack 20 s; Soul Desecration permanent x2; Dark Prayers 1 spark per hit); engine reads `derived.investitureAvg` | Puppet damage counted as raw magnitude in the sim; Investiture stacks feed Risky Investment via the mechanic effect. | puppet does not yet use its own stat sheet |
| Standing DoTs + power modifiers | Implemented in the simulator, LOCAL ONLY | `dotBlock` (Hellish Rebuke, Hadar's Grasp, Flames of Phlegethos), "AxB" pulses (Blades, Dreadtheft), `hazard` (Hellfire Ring), `summon.imps` (Brood), `comboMagnitudes` (Eldritch Blast), Creeping Death feat; `powerMods` for No Pity No Mercy and Double Scorch are applied inside the sim | Ticks scheduled on their own clocks; DoTs refresh, never stack; Creeping Death 5 stacks x 25 per 2 s. | powerMods are Hellbringer-specific branches, not a generic applier yet |
| Enemy missing-health scaling | Implemented (2026-09-11) | `toon-forge.html` class-feature loop: `enemyMissingHealthScaled` (Executioner's Gift) scales the conditional amount by the enemy-health slider; `toon-forge-rotation.js` `magnitudeRange` (Killing Flames) | 0 at 100% enemy health -> full at 0%, linear. | n/a |
