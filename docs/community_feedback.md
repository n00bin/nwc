...
**Status:** **BUILT for gear + weapons (2026-06-23).** Engine support is generic (any `SLOTS` entry with a `label` can be pinned — `js/optimizer-local.js`: `OPT_SLOTS = SLOTS.filter(not pinned)`, used by `runGreedy` + `perturb`); `state.pinned` (Set of slot labels) is serialized + carried in share links; weapons lock as a Main Hand + Off Hand pair. **TWO lock surfaces, shared `state.pinned`:** (1) builder — a 🔒 toggle on each filled gear slot (`toon-forge.html` `renderGear`); (2) optimizer results — a **🔒 keep mine** button on changed gear/weapon cards, batched into the same Re-run as ✕/↓ (`loopPinned` seeds from builder locks at `onClick`, overrides `state.pinned` each `runPass`, and feeds back so accepted locks show in the builder). **Discoverability:** results-panel tip line + a builder hint under "Equip your gear". **Pre-run "Must-have gear" (2026-06-23):** the optimize dialog has a "Must-have gear" section — pick pieces (two-stage slot→item picker reusing the builder's gear picker) that get **equipped + locked on run**, so you can build around a piece you haven't equipped yet (covers both "keep this" and "I got a new drop, show me what it does"). `applyMustHaves()` writes the chosen gear into `state` + `loopPinned` and re-snapshots `loopOriginal`; the optimizer overlay (z-index 99999) tucks to 500 while the picker (600) is open via `tuckOptimizerUnderPicker()`. This closes the gap where a user runs the optimizer before equipping the piece they want kept. **Design rationale (n00b 2026-06-23):** exclusions are NOT persisted across runs on purpose — each run is for *inclusion* (lock something new / test a new drop), so remembering "don't have" would wrongly keep excluding gear the player just acquired. REMAINING: results-panel lock only shows on *changed* gear cards (unlock an already-kept slot from the builder); extend lock UI to enchants / companions / artifacts / mounts (engine already supports them once those slots get a `label` + toggle). n00b locked **Option 1 (per-slot Pin/Lock)** on 2026-06-20.
